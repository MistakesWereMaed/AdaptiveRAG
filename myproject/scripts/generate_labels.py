...
def _choose_label(
    ...
    """
    Choose best strategy using:
      1. exact match first,
      2. then F1,
      3. cost-aware tie-break if top strategies are close.
    ...
    # If top strategies are close, prefer cheaper strategy.
...
def main():
    ...
    for start in tqdm(range(0, total, batch_size), desc="Labeling"):
        ...
        for i in range(actual_batch_size):
            scores = {
                strategy: batch_eval[strategy]["f1"][i]
                for strategy in STRATEGY_TO_LABEL
            }

            ems = {
                strategy: batch_eval[strategy]["em"][i]
                for strategy in STRATEGY_TO_LABEL
            }

            accs = {
                strategy: batch_eval[strategy]["acc"][i]
                for strategy in STRATEGY_TO_LABEL
            }
    ...
    stats = {
        ...
        "unlabeled_pct": unlabeled_count / total if total else 0.0,
    ...
    for strategy, dist in stats["label_distribution"].items():
        print(
            ...
            f"{dist['pct_of_total']:.2%} of total)"
        )
    ...
    print(f"Unlabeled:        {unlabeled_count} ({stats['unlabeled_pct']:.2%})")
