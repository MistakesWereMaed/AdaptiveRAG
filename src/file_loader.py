...
def load_records(path: Union[str, Path]) -> List[QAItem]:
    ...
    Preserves dataset metadata needed for dataset-specific retrieval routing.
    """
    ...
    for i, record in enumerate(raw):
        if isinstance(record, dict):
            ...
            a = record.get("gold") or record.get("answer") or record.get("answers")
            ...
            raw_id = record.get("id", record.get("_id", record.get("source_id", i)))
            ...
            # Critical routing fields.
            if dataset is not None:
                metadata["dataset"] = str(dataset)

            if question_type is not None:
                metadata["question_type"] = str(question_type)

            # Useful answer/scoring fields.
            if isinstance(answers, list):
                metadata["answers"] = answers

            if record.get("supporting_titles"):
                metadata["supporting_titles"] = record["supporting_titles"]

            if record.get("supporting_facts"):
                metadata["supporting_facts"] = record["supporting_facts"]

            if record.get("source_split") is not None:
                metadata["source_split"] = record["source_split"]

            if record.get("sample_index") is not None:
                metadata["sample_index"] = record["sample_index"]

            if record.get("source_id") is not None:
                metadata["source_id"] = str(record["source_id"])
            ...
            # If QAItem supports dynamic attrs, attach dataset directly.
            # If it is a strict dataclass without this field, this safely does nothing.
            try:
                setattr(item, "dataset", str(dataset) if dataset is not None else None)
            except Exception:
                pass
    ...
    missing_dataset = sum(
        1 for ex in examples if not getattr(ex, "dataset", None) and not ex.metadata.get("dataset")
    )

    if missing_dataset:
        print(
            f"[preprocessing] Warning: {missing_dataset}/{len(examples)} records have no dataset field",
            flush=True,
        )
