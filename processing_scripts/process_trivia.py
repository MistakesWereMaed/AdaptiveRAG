...
def _existing_input_path(input_directory: str, set_name: str) -> Optional[str]:
    candidates = [
        ...
        os.path.join(input_directory, f"trivia-{set_name}.json"),
        os.path.join(input_directory, f"trivia-{set_name}.jsonl"),
        os.path.join(input_directory, f"biencoder-trivia-{set_name}.json"),
        os.path.join(input_directory, f"biencoder-trivia-{set_name}.jsonl"),
        os.path.join(input_directory, f"biencoder-triviaqa-{set_name}.json"),
        os.path.join(input_directory, f"biencoder-triviaqa-{set_name}.jsonl"),
    ]
...
def _load_records(path: str) -> List[Dict[str, Any]]:
    ...
    for key in ("data", "records", "examples"):
        if isinstance(payload, dict) and isinstance(payload.get(key), list):
            return payload[key]
    raise ValueError(f"Unsupported raw TriviaQA structure: {path}")
...
def _normalize_answers(raw_instance: Dict[str, Any]) -> List[str]:
    ...
    # Some TriviaQA files store aliases under answer object fields.
    if isinstance(answers, dict):
        aliases = []
        for key in ("aliases", "normalized_aliases", "human_answers", "value"):
