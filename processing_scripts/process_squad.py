...
def _existing_input_path(input_directory: str, set_name: str) -> Optional[str]:
    candidates = [
        ...
        os.path.join(input_directory, f"squad1-{set_name}.json"),
        os.path.join(input_directory, f"squad1-{set_name}.jsonl"),
        os.path.join(input_directory, f"biencoder-squad1-{set_name}.json"),
        os.path.join(input_directory, f"biencoder-squad1-{set_name}.jsonl"),
        os.path.join(input_directory, f"train-v1.1.json") if set_name == "train" else "",
        os.path.join(input_directory, f"dev-v1.1.json") if set_name in {"dev", "test"} else "",
    ]
        ...
        for article in payload["data"]:
            ...
            for paragraph in article.get("paragraphs", []):
                ...
                for qa in paragraph.get("qas", []):
                    answers = [answer.get("text", "") for answer in qa.get("answers", [])]
                    records.append(
                        {
                            "id": qa.get("id"),
                            "question": qa.get("question"),
                            "answers": answers,
                            "positive_ctxs": [{"title": title, "text": context}],
                        }
                    )
        return records

    for key in ("records", "examples"):
        if isinstance(payload, dict) and isinstance(payload.get(key), list):
            return payload[key]

    raise ValueError(f"Unsupported raw SQuAD structure: {path}")
...
def _normalize_answers(raw_instance: Dict[str, Any]) -> List[str]:
    ...
    if isinstance(answers, dict):
        answers = answers.get("text") or answers.get("spans") or []
